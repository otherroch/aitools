...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=1024,
        help=(
            "Output resolution in pixels.  When --crop-height is not specified\n"
            "this is the square output pixel size of the photo or video segment.\n"
            "When --crop-height is specified this is the width in pixels."
        ),
    )
    parser.add_argument(
        "--crop-height",
        ...
        default=None,
        help=(
            "Height (in pixels) of the output photo or video segment.  If specified\n"
            "then --crop-size specifies the width in pixels.  If not specified\n"
            "then --crop-size specifies the square output pixel size."
        ),
    )
    parser.add_argument(
        "--extract-only",
        action="store_true",
        help=(
            "Just extract frames without doing face recognition.  Simply extract\n"
            "every N-th frames from each video."
        ),
    )
    parser.add_argument(
        "--no-classify",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "  photo  – face-cropped PNG frames (default, existing behaviour).\n"
            "  video  – MP4 segments that each contain exactly one unique person."
        ),
    )
    parser.add_argument(
        "--max-segment-length",
        ...
        default=30.0,
        help=(
            "Maximum length of each output video segment in seconds "
            "(default: 30).  Longer segments are split at this boundary.\n"
            "Only used when --output-type video is specified."
        ),
    )
    parser.add_argument(
        "--min-segment-length",
        ...
        default=2.0,
        help=(
            "Minimum length of each output video segment in seconds "
            "(default: 2).  Shorter segments are discarded.\n"
            "Only used when --output-type video is specified."
        ),
    )
