...
def load_qwen38_model_and_processor(args):
    """
    Load Qwen3.8 model and processor once and reuse across videos.
    In threaded batch mode, this lets all threads share the same CUDA weights.

    The model architecture is resolved automatically via
    ``AutoModelForMultimodalLM`` so any transformers-registered Qwen3.8
    variant (e.g. the Inferact/Qwen3.8-27B-NVFP4 quantized checkpoint) works
    without hard-coding a specific class.
    """
    global _SHARED_MODEL, _SHARED_PROCESSOR

    if _SHARED_MODEL is not None and _SHARED_PROCESSOR is not None:
        logger.debug("load_qwen38_model_and_processor: reusing cached model/processor")
        return _SHARED_MODEL, _SHARED_PROCESSOR
    ...
    logger.debug("load_qwen38_model_and_processor: model_path=%s  quant=%s  attn=%s",
                 model_path_local, getattr(args, "quant", None), getattr(args, "attn", None))
    print("model_path=", model_path_local)
    ...
    if args.processor is not None:
        ...
        logger.debug("load_qwen38_model_and_processor: using custom processor path %s", processor_path_local)
    ...
    if getattr(args, "half_cpu", False):
        ...
        print(f"Number of CPU cores in the system: {cpu_count}")
        half_cpu_count = cpu_count // 2
        logger.debug("load_qwen38_model_and_processor: limiting to %d CPU threads", half_cpu_count)
        os.environ["MKL_NUM_THREADS"] = str(half_cpu_count)
        os.environ["OMP_NUM_THREADS"] = str(half_cpu_count)
        torch.set_num_threads(half_cpu_count)
    ...
    now = datetime.now()
    current_time = now.strftime("%H:%M:%S")
    logger.debug("load_qwen38_model_and_processor: start time %s", current_time)
    print(f"✅ start time (model load): {current_time}")
    ...
    # Load model using AutoModelForMultimodalLM (auto-resolves the Qwen3.8 architecture)
    ...
    if args.optimize:
        logger.debug("load_qwen38_model_and_processor: compiling model with torch.compile")
        model = torch.compile(
            model,
            mode="reduce-overhead",
            fullgraph=True,
    ...
    patch = _patch_size_for_model(model_path_local)
    logger.debug(
        "load_qwen38_model_and_processor: patch=%d  min_pixels=%d  max_pixels=%d",
        patch, args.min_pixels * patch * patch, args.max_pixels * patch * patch,
    )
    processor = AutoProcessor.from_pretrained(
    ...
    logger.debug("load_qwen38_model_and_processor: model loaded")
    print("model loaded")
