...
def load_qwen38_model_and_processor(args):
    ...
    Uses AutoModelForMultimodalLM and AutoProcessor (as indicated by the
    model card for Inferact/qwen3.8-27b-nvfp4).
    In threaded batch mode, this lets all threads share the same CUDA weights.
    """
    global _SHARED_MODEL, _SHARED_PROCESSOR
    ...
    # Load model using AutoModelForMultimodalLM (standard for Qwen3.8 NVFP4)
    model = AutoModelForMultimodalLM.from_pretrained(
        model_path_local,
        device_map="auto",
        torch_dtype="auto",
